...
static char *generate_file_name(const char *name) {
    ...
    size_t max_buffer_size =
        ...
        strlen(name) + strlen(".0") + strlen(".") + strlen(".") + strlen(lib_suffix) + strlen(lib_prefix) + 1;
    ...
    snprintf(versioned_file_name, max_buffer_size, "%s%s.%s.0", lib_prefix, name, lib_suffix);
...
k4a_result_t dynlib_create(const char *name, dynlib_t *dynlib_handle) {
    ...
    char *versioned_name = generate_file_name(name);
