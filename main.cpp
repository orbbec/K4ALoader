...
// C++接口开流
// C++ Interface Open Stream
void openAllStreamCPlus()
{
	// 输入动态库所在文件夹路径, 加载动态库
	// Input the path to the folder where the library is located, and load the library.
	k4aRuntimeHandle* klHandle = k4aloader_create_runtime_handle("D:\\k4aLoader\\k4aLoader\\Test\\wrapper");

	if (klHandle != nullptr)
	{
		k4a::capture capture;
		// 静态函数调用需要输入k4a_runtime_handle作为参数
		// Static function calls require k4a_runtime_handle as an argument.
...
void openAllStreamC()
{
	...
	if (instance != NULL)
	{
		...
		// Use k4a_runtime_handle to make function calls to get the number of currently connected devices.
		...
		if (count > 0)
		{
			...
			// 配置开流config
			// Configure the open stream config
			k4a_device_configuration_t config = K4A_DEVICE_CONFIG_INIT_DISABLE_ALL;
			config.color_format = K4A_IMAGE_FORMAT_COLOR_MJPG;
			config.color_resolution = K4A_COLOR_RESOLUTION_1080P;
			config.depth_mode = K4A_DEPTH_MODE_NFOV_UNBINNED;
			...
			// 关流、关闭设备
			// Shut down the flow, turn off the device
			if (device != NULL)
			{
...
// C++ opens Meaga and Azure Kinect at the same time  TODO:待修改二选一打开、函数名字修改
void bothDeivceOpen()
{
	// 输入动态库所在文件夹路径, 加载动态库
	// Input the path to the folder where the library is located, and load the library.
	...
	if (klHandleWR != nullptr && klHandleAK != nullptr)
	{
		...
		if (countWR > 0 && countAK > 0) {
			...
			// Explicitly pass in the SDK's handle to open a different device.
			...
			// 关闭设备连接, 同时会对使用的k4a_runtime_handle的引用计数减1
			// Closes the device connection and decrements the reference count of k4a_runtime_handle by 1.
			warDevice.stop_cameras();
			warDevice.close();
			akDevice.stop_cameras();
			akDevice.close();
		}
		// 释放动态库连接, 需注意只有当k4a_runtime_handle引用计数为0时才会被彻底释放掉
		// Release the dynamic library link, note that it will only be completely released when the k4a_runtime_handle reference count is zero.
